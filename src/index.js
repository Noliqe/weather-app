...
async function displayForecast (forecastData) {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    const forecastIcon = await fetch('http://openweathermap.org/img/wn/' + forecastIconCode + '@2x.png', { mode: 'cors'});  
...
async function getWeather(data) {
  ...
  try {
    ...
    // fetch image 
    const icon = await fetch('http://openweathermap.org/img/wn/' + weatherIcon + '@2x.png', { mode: 'cors'});    
